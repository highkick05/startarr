...
app.post("/api/scrape-metadata", async (req: any, res) => {
  ...
  try {
    ...
    if (finalTitle) {
      const sanitized = finalTitle.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
      if (sanitized) {
        resolvedIcons.unshift(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${sanitized}.png`);
        resolvedIcons.unshift(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/svg/${sanitized}.svg`);
        if (sanitized.includes('-')) {
           const noDash = sanitized.replace(/-/g, '');
           resolvedIcons.unshift(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${noDash}.png`);
           resolvedIcons.unshift(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/svg/${noDash}.svg`);
        }
      }
    }
    ...
    try {
      ...
      const fallbackIcons = [];
      if (fallbackTitle) {
        const sanitized = fallbackTitle.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
        if (sanitized) {
          fallbackIcons.push(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${sanitized}.png`);
          fallbackIcons.push(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/svg/${sanitized}.svg`);
          if (sanitized.includes('-')) {
             const noDash = sanitized.replace(/-/g, '');
             fallbackIcons.push(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${noDash}.png`);
             fallbackIcons.push(`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/svg/${noDash}.svg`);
          }
        }
      }
